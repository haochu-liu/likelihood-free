...
if __name__ == "__main__":
    ...
    nre_coverage_results = np.full((2, len(budgets)), np.nan)
    nre_mean_error_results = np.full((theta_test.shape[0], len(budgets), 2), np.nan)
    ...
    for i in range(len(budgets)):
        ...
        nre_coverage_results[0, i] = coverage_95_with_L
        nre_coverage_results[1, i] = coverage_95_without_L
        nre_mean_error_results[:, i, 0] = mean_error_with_L
        nre_mean_error_results[:, i, 1] = mean_error_without_L
        ...
        np.savetxt(str(data_path / "benchmark" / "nre_coverage_results.csv"), nre_coverage_results, delimiter=",")
