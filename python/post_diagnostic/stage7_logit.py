...
if __name__ == "__main__":
    ...
    for index_seed in range(len(seeds)):
        ...
        for i in range(len(budgets)):
            ...
            theta_train_logit = bounded_to_logit(
                theta=theta_train,
                low=low,
                high=high,
            )
            ...
            inference_benchmark = NPE_C(density_estimator=neural_posterior, device=torch_device)
            density_estimator_benchmark = inference_benchmark.append_simulations(theta_train_logit, x_train).train(
                max_num_epochs=500,
            ...
            file_name = f"npe_post_stage7_seed{seed}_sim{n_sim}.npy"
