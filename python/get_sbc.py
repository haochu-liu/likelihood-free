...
if __name__ == "__main__":
    ...
    theta_sbc, x_sbc = simulate_for_sbi(
        simulator=simulator, proposal=prior, num_simulations=num_sbc_samples, num_workers=8
    )

    # Reset directory and save simulation results
    output_file = data_path
    np.savetxt(output_file / 'theta_sbc.csv', theta_sbc.cpu().numpy(), delimiter=",")
    np.savetxt(output_file / 'x_sbc.csv', x_sbc.cpu().numpy(), delimiter=",")
