...
E_W.fix_var <- matrix(NA, nrow=length(n), ncol=length(N))
colnames(E_W.fix_var) <- N
rownames(E_W.fix_var) <- n

var_W.fix_var <- matrix(NA, nrow=length(n), ncol=length(N))
  ...
  for (j in 1:length(n)) {
    ...
    results_k <- future_lapply(1:100, function(k) {
      ...
      list(
        ...
        post_d = approx(kde_res$x, kde_res$y, xout=lambda)$y
      )
    }, future.seed = TRUE)

    acc_rate.fix_var[j, i] <- mean(sapply(results_k, `[[`, "acc"))
    ess.fix_var[j, i]      <- mean(sapply(results_k, `[[`, "ess"))
    err_mean.fix_var[j, i] <- mean(sapply(results_k, `[[`, "err"))
    est_post <- sapply(results_k, `[[`, "post_d")

    true_post <- dgamma(lambda, shape=alpha+sum(y_obs[1:N_val]), rate=beta+N_val)
