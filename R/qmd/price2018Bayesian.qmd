...
paper_mcmc <- data.frame(n=c(1, 2, 5, 6, 7, 10, 15, 20),
                         ...
                         ess.est_var=NA,
                         norm_ess.est_var=NA)
err_mean.fix_var <- rep(0, 8)
err_mean.est_var <- rep(0, 8)

for (i in 1:nrow(data_list$acc.fix.var)) {
  paper_mcmc$acc_rate.fix_var[i] <- mean(data_list$acc.fix.var[i, ])
  ess_vec <- rep(NA, 10)
  for (j in 1:10) {
    ess_vec[j] <- as.numeric(effectiveSize(
      as.mcmc(data_list$chain.fix.var[, (i-1)*10+j])
    ))
    err_mean.fix_var[i] <- err_mean.fix_var[i] +
      abs(posterior_mean - mean(data_list$chain.fix.var[50000:100000, (i-1)*10+j]))
  }
  paper_mcmc$ess.fix_var[i] <- mean(ess_vec)
  paper_mcmc$norm_ess.fix_var[i] <- mean(ess_vec) / paper_mcmc$n[i]
  err_mean.fix_var[i] <- err_mean.fix_var[i] / 10
  ...
  if (i > 1) {
    ...
    for (j in 1:10) {
      ess_vec[j] <- as.numeric(effectiveSize(
        as.mcmc(data_list$chain.est.var[ ,(i-1)*10+j])
      ))
      err_mean.est_var[i] <- err_mean.est_var[i] +
        abs(posterior_mean - mean(data_list$chain.est.var[50000:100000, (i-1)*10+j]))
    }
...
print(paper_mcmc)
print(err_mean.fix_var)
print(err_mean.est_var)
```
...
abline(v = posterior_mean, col = "red", lwd = 2)
lines(density(data_list$chain.est.var[, 15]), col="green", lwd=3)
...
### Fix T = 100000 and change n and N

```{r}
#| echo: true
load("../../data/toy_fix_var.RData")

acc_df <- as.data.frame(round(toy_fix_var$acc_rate, 3))
acc_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
  tab_header(title = "Acceptance rate")
```

```{r}
#| echo: true
var_log_df <- as.data.frame(round(toy_fix_var$var_log, 3))
var_log_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
  tab_header(title = md("$\\text{var}(\\log p_{SL}(y|\\lambda=30))$"))
```

```{r}
#| echo: true
load("../../data/toy_like_fix_var.RData")

est_df <- as.data.frame(round(toy_like_fix_var$est, 3))
est_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
  tab_header(title = md("$\\text{var}(p_{SL}(y|\\lambda=30)) / \\text{E}(p_{SL}(y|\\lambda=30))^2$"))
```

```{r}
#| echo: true
norm_ess_df <- as.data.frame(round(toy_fix_var$norm_ess))
norm_ess_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
  tab_header(title = "Normalized ESS")
```

```{r}
#| echo: true
load("../../data/poisson_toy_table.RData")
acc_rate_df <- as.data.frame(round(poisson_toy_table$acc_rate, 3))
acc_rate_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
...
```{r}
#| echo: true
norm_ess_df <- as.data.frame(round(poisson_toy_table$norm_ess, 3))
norm_ess_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
...
```{r}
#| echo: true
var_log_df <- as.data.frame(round(poisson_toy_table$var_log, 1))
var_log_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
...
```{r}
#| echo: true
est_df <- as.data.frame(round(poisson_toy_table$est, 1))
est_df %>%
  gt(rownames_to_stub = TRUE) %>% 
  tab_stubhead(label = "n") %>%
  tab_spanner(label = "dim(y)", columns = everything()) %>%
