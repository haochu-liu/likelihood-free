...
paper_normalized_ess_fix <- rep(NA, length(mcmc_quality1$n))
paper_normalized_ess_est <- rep(NA, length(mcmc_quality1$n))
names(paper_normalized_ess_fix) <- mcmc_quality1$n
names(paper_normalized_ess_est) <- mcmc_quality1$n

for (i in 1:length(mcmc_quality1$n)) {
  ess <- as.numeric(effectiveSize(as.mcmc(data_list$chain.fix.var[, i])))
  paper_normalized_ess_fix[i] <- ess / mcmc_quality1$n[i]
  
  if (i > 1) {
    ess <- as.numeric(effectiveSize(as.mcmc(data_list$chain.est.var[, i])))
    paper_normalized_ess_est[i] <- ess / mcmc_quality1$n[i]
  }
}

print(paper_normalized_ess_fix)
print(paper_normalized_ess_est)
```
