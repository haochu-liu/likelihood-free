...
```{r}
#| echo: true
q_sigma <- matrix(0.1, nrow=1, ncol=1)
theta_seq <- SL_MCMC(M, iter, obs, init_theta, prior_func, sample_func, q_sigma,
                     acc_rate=TRUE)
```
...
```{r}
#| echo: true
theta_smc <- SL_SMC(M, alpha, N, theta_d, obs, prior_sampler, prior_func,
                    sample_func, sigma, gamma_history=TRUE)
```

```{r}
#| echo: true
density_mcmc <- density(theta_seq$theta[1, 40000:50000])
density_am <- density(theta_seq_AM$theta[1, 40000:50000])
density_smc <- density(theta_smc$theta[1, ])
...
plot(density_mcmc,
     ...
     ylab = "Density",
     col = "red", lwd = 2)
lines(density_am, col = "blue", lwd = 2)
lines(density_smc, col = "green", lwd = 2)
lines(x_values, y_values, col = "black", lwd = 2, lty = 2)
legend("topright",
       legend = c("BSL-MCMC", "BSL-AM", "BSL-SMC", "Exact posterior"),
...
for (r in 1:R) {
  time_result <- system.time(
  chain_r <- SL_SMC(M, alpha, N, theta_d, obs, prior_sampler, prior_func,
                    sample_func, sigma)
  )
  time_vec[r] <- time_result["elapsed"]
  ql_vec[r] <- quantile(chain_r[1, ], probs=0.25)
  qu_vec[r] <- quantile(chain_r[1, ], probs=0.75)
  e_vec[r] <- mean(chain_r[1, ])
  
  if (r %% 10 == 0) {print(paste0("Finish iteration ", r))}
}
efficiency_df$`time(sec)`[3] <- mean(time_vec)
efficiency_df$`V_QL*sec`[3] <- var(ql_vec) * mean(time_vec)
efficiency_df$`V_QU*sec`[3] <- var(qu_vec) * mean(time_vec)
efficiency_df$`V_E*sec`[3] <- var(e_vec) * mean(time_vec)
```
...
### Chain length and ESS

```{r}
#| echo: true
chain <- SL_MCMC(M, iter, obs, init_theta, prior_func, sample_func,
                 0.1)
ess_different <- rep(NA, 50)
for (i in 1:50) {
  mcmc_chain <- as.mcmc(chain[1, ((i-1)*1000):(i*1000)])
  ess_different[i] <- effectiveSize(mcmc_chain)
}
plot(1:50, ess_different)
```

```{r}
#| echo: true
chain <- SL_AM(M, iter, 5000, obs, init_theta, prior_func, sample_func,
               0.1, 0.01)
ess_different <- rep(NA, 50)
for (i in 1:50) {
  mcmc_chain <- as.mcmc(chain[1, ((i-1)*1000):(i*1000)])
