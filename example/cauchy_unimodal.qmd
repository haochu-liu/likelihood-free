...
q_sigma <- matrix(1, nrow=1, ncol=1)
theta_mcmc <- ABC_MCMC(tol, iter, s_obs, gaussian_kernel, theta_sigma,
...
```{r}
#| echo: true
tol_n <- length(theta_smc$tol)
par(mfrow = c(1, 3))
plot(1:tol_n, theta_smc$tol, type = "p",
     xlab = "Iteration",
     ylab = "gamma")
plot(1:tol_n, exp(theta_smc$ess), type = "l",
     ylim = c(0, exp(theta_smc$ess[1])),
     xlab = "Iteration",
     ylab = "ESS")
plot(1:tol_n, exp(theta_smc$cess), type = "l",
     ylim = c(0, exp(theta_smc$cess[1])),
     xlab = "Iteration",
     ylab = "CESS")
par(mfrow = c(1, 1))
...
```{r}
#| echo: true
theta_density <- density(theta_smc$theta[1, ,tol_n],
                         bw = 0.5,
                         weights = exp(theta_smc$weight[tol_n, ]))
