...
Let's choose different values of $M$ to see how it affects the posterior given by BSL. In the same example, we consider $M = 2, 5, 10, 15, 20, 25, 30, 50, 100$. And plot their density curves.

```{r}
#| echo: true
M_df <- data.frame(values = c(2, 5, 10, 15, 20, 25, 30, 50, 100),
                   acc = NA,
                   ESS = NA,
                   var = NA)
theta_matrix <- matrix(NA, nrow=9, ncol=50000)
